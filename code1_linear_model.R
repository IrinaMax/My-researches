...
# looking for NA or missing data
...
# Add salary to the train table to make full train 'tn'
...
# tn %>% dim
hist(tn$salary)
t.test(tn$salary)  #alternative hypothesis: true mean is not equal to 0

# Corralation matrix to find out what variables better to use for my model
# I do not need jobID , it is just numbers of rows and not relative for sure
...
tt.salary %>% head
# jobId                  salary
# 1 JOB1362685407687    116
# 2 JOB1362685407688     92
# 3 JOB1362685407689    167
# 4 JOB1362685407690    105
# 5 JOB1362685407691    119
# 6 JOB1362685407692    158
tt.salary %>% summary
write.csv(tt.salary, "test_salaries1.csv")


#_________________________________________________end linear mod______________________________________
#________________________________________analyling accuracy____________________________________________________
...
mse_test_value_lm <- mean((pl$predic-pl$actual)^2)
mse_test_value_lm
#[1] 2613.805688
...
# jobType                  7 535091060  76441580 198822.0382 < 0.000000000000000222 ***
#   degree                   4  93941250  23485313  61084.5264 < 0.000000000000000222 ***
#   major                    8  11874186   1484273   3860.5459 < 0.000000000000000222 ***
#   yearsExperience          1 210187970 210187970 546692.0049 < 0.000000000000000222 ***
#   industry                 6 130422811  21737135  56537.5746 < 0.000000000000000222 ***
#   milesFromMetropolis      1 133098217 133098217 346184.0893 < 0.000000000000000222 ***
#   Residuals           999972 384461604       384
# ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
#
apa.cor.table(tn[,-1],table.number = 2,show.conf.interval = F)
...
apa.reg.table(lm(salary ~ jobType+degree+ major+yearsExperience+industry +milesFromMetropolis, tn),table.number = 3)
...
#______________________________________________End of ANOVA Tables__________________________________

# Why I choose the linear model? I check it out on train with existing salary.
# I split the merged table "tn" with salary for train & test to fir lm and than check out the accurasy for my model compare existing salary and predicted of mytest
set.seed(123)
...
plot(df.lm[sample(nrow(df.lm),1000),],col = "pink", main = "Linear model Prediction on subset test")
